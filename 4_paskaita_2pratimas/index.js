...
const data = require("./data"); // require padeda importuoti duomenis
const slugify = require("slugify"); //kad galetų skaityti lietuviškas raides
const port = 3000;
...
app.get("/products/categories/:category", (req, res) => {
  ...
  const filteredProducts = data.filter(
    (product) =>
      slugify(product.category.toLowerCase()) === category.toLowerCase() //slugify padeda skaityti lietuviškas raides
  );
...
app.get("/stock", (req, res) => {
  const filteredProducts = data.filter((product) => product.stock < 10); //rodyti visus kurių likučiai mažiau negu 10
  const stocks = filteredProducts.map((product) => {
    return {
      name: product.name,
      stock: product.stock,
    }; //map'as padaro pateikimą kitokį
  });
  res.send(stocks);
});
...
app.post("/products", (req, res) => {
  ...
  const isIdExist = data.some((product) => product.id === newProduct.id); //some funkcija reiskia ar viena is salygu atitinka -> some grazina true arba false
