...
app.post("/login", (req, res) => {
  ...
  if (foundedUser !== undefined) {
    // rado 
    let submittedPassword = req.body.password; // rado slaptazodi
    let storedPassword = foundedUser.password; // rado slaptazodi
    // rado slaptazodi === rado slaptazodi
    // rokas123 === rokas123!
    if (submittedPassword === storedPassword) {
      res.send({ message: "Sveikinu prisijungus!", approved: true });
    } else {
