...
app.get('/movies/genre/:title', async (req, res) => {
  try {
    ...
    const data = await con
      .db('ManoDuomenuBaze')
      .collection('Movies')
      .find({ genre: title }) // ištraukia VISUS objektus pagal tam tikrą lauką pvz. genre
      .toArray();
    await con.close();
    res.send(data);
  } catch (error) {
...
// asc - ascending - rikiuoja didėjimo tvarka
// dsc - descending - rikiuoja mažėjimo tvarka
